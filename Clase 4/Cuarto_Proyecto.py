...
while(cant_intentos > 0):
    ...
    if(numero > 100 or numero < 1): 
        ...
        cant_intentos -= 1
        if numero > valor_aleatorio: print(f'Tu número es MAYOR, vuelve a probar suerte.\n Te quedan {cant_intentos} intentos.')
        
        elif numero < valor_aleatorio: print(f'Tu número es MENOR, vuelve a probar suerte.\n Te quedan {cant_intentos} intentos.')
        
        else: 
            print(f'Excelente {nombre}!! Lo has conseguido en {8-cant_intentos} intentos. \n Gracias Por Jugar')
            break

if(numero != valor_aleatorio): print(f'Lo siento {nombre} el número era {numero} y te has quedado sin intentos. Has Perdido :(')